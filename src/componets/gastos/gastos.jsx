...
const Gastos = () => {
  ...
  //   const value = optionObj[0].Observaciones?.trim()
  // ? optionObj[0].Observaciones
  // : "No hay anticipos";
  // console.log(optionObj,"ddddd")
  //   // const value = `${totalAnt[0].IdCentroCostos}-${optionObj.sku} ${optionObj.DetalleConcepto} ${nomProyect} ${optionObj.NumeroComprobante}`;
  ...
  const handleOptionSelect = (optionObj) => {
  const observaciones = optionObj?.Observaciones?.trim();

  const value = observaciones || "No hay anticipos";
  ...
  totalAnt.map((option, index) => (
    ...
    >
      ...
      >
        {option.Observaciones || "Sin observaciones"}
      </span>
    </button>
  ))
) : (
  <div className="p-2 text-gray-500">No hay anticipos para este proyecto.</div>
)}
      </>
    );
  };
...
>
  ...
  const handleOptionSelectProyecto = async (option) => {
  const value = option.Nombre;
  const docEmpleado = localStorage.getItem("doc_empleado");

  try {
    const { data } = await axios.post("/proyect/anticipo", {
      doc: docEmpleado,
      op: option.OP,
    });

    // Guarda los anticipos encontrados
    todosAnticipos(data);

    if (data && data.length > 0) {
      // Hay anticipos disponibles
      setSelectedOptions([]);
      setPrepayment(null);
      // setIsOpen(true); // Abre el selector de anticipos
    } else {
      // No hay anticipos para este proyecto
      setSelectedOptions(["No hay anticipos"]);
      setPrepayment(null);
      setIsOpen(false);
      SetJustSelected(true);
    }
    ...
    console.error("Error consultando anticipos:", error);
              ...
              <div className="">
                <div className="">
                  ...
                  <input
                    ...
                    placeholder="Escribe aquí la descripción del envío"
                  ></input>
                </div>
